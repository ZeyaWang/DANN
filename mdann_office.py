...
class disc(nn.Module):
    def __init__(self, model_name):
        ...
        self.logit = nn.Linear(512, n_disc)
    def forward(self,x,alpha):
        x = ReverseLayerF.apply(x, alpha)
        x = F.relu(self.fc1(x))
        x = self.logit(x)
...
def DiscLoss(preds, discriminator_y, one_hot_or_softmax):
    '''
    Args:
    preds: [B, n_disc]: logits for binary loss between source and targret
    discriminator_y: [B, n_disc]; binary lable for source or domain for batch samples
    one_hot_or_softmax: [B, n_disc]: one hot encoding or probablity scores for disc
    branches (i.e. classes for our first test)
    '''
    loss_unweight = criterion(preds, discriminator_y) # return tensor with [B, n_disc]
    loss_weight = one_hot_or_softmax*loss_unweight
    loss = loss_weight.sum(1)
    loss = loss.mean()
    return loss 
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for l in trange(len_dataloader, leave=False):
            ...
            target_pred_detach = softmax(target_pred).detach()
            source_y_one_hot = one_hot(source_y, N_CLASS).float() # one hot encoding
            discriminator_class = torch.cat([source_y_one_hot, target_pred_detach], dim=0) # [B, N_class]
            #disc_output = disc_model(discriminator_x, alpha).squeeze()
            disc_output = disc_model(discriminator_x, alpha)
            #disc_loss = criterion(disc_output, discriminator_y)
            disc_loss = DiscLoss(disc_output, discriminator_y, discriminator_class)
